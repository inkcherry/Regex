...
NFA::NFA(Node* Tree)   //
{
	...
	end_status->IsFinal = true;  //最终状态为true
	E2NFA();
    //整体的开始状态不能作为结束状态，防止接收任意字符使其匹配结束
	 //find_end_status();
}
NFA::~NFA()
{
...
pair<Status*, Status*> NFA::gen_repeat_0(Node* node)   //*型重复 0次以上重复
{
	Repeat_Node* repeat_node = static_cast<Repeat_Node*>(node);     //声明一个node型 用基类的指针node 初始化repeat_node
	Status* s_start = new Status(true);                //这是重复节点的最终状态，也是连接两条E边的状态
	Status* s_end = s_start;                                      //这两部表示 起始状态和终止状态重复
	Status* _s_start = nullptr;
	Status* _s_end = nullptr;
	pair<Status*, Status*> s_child = gen_status(repeat_node->node);   //子图开始 结束
...
pair<Status*, Status*> NFA::gen_repeat_1(Node* node)   //可选+
{
...
pair<Status*, Status*> NFA::gen_and(Node* node)             //处理and
{
	...
	for (auto s : *(and_node->pool))   // 遍历这个  pool
	{
		...
		s_end->IsFinal = false;
		make_edge(s_end,tmp.first);  //连接E边
		s_end = tmp.second;
	}
/*	s_end->IsFinal= true;       */        //然后给最后一个为true 也就是整个and的最终状态
	return make_pair(s_start, s_end);
}
pair<Status*, Status*> NFA::gen_or(Node* node)    //处理或
{
	...
	for (auto s : *(or_node)->pool)
	{
		...
		//s_end->IsFinal = true;       //这里设置最终的状态是true  =>true
	}
...
Edge* NFA::make_edge(Status* status1, _MatchContent content, Status* status2,bool isAdd)
{
	if (isAdd&&status1->IsFinal)
		status1->IsFinal = false;                                           //设置的新边 所有的新边均设置为false
...
void NFA::E2NFA() //ENFA转化为NFA （这里是将E边删除）
{
	...
	for (auto status : valid_status) //寻找有效状态的E-closure,并将其延伸的有效边复制到有效状态上
	{
		...
		{
			Status* head = uncomplete_status.front();     //弹出队首元状态
			uncomplete_status.pop();
			for (auto edge : head->OutEdges)           //找改状态出边
			{
				if (_isEedge(edge)) //以该状态为起始的E边
				{   
					if (edge->End->IsFinal)   //如果状态一个闭包可以到达最终状态
						status->IsFinal = true;
		...
		{
			edges_add.push_back (make_edge(status, edge->MatchContent, edge->End,false)); //需要复制的边 //false不改变边的任何变量
